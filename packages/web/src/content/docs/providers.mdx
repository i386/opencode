...
For OpenAI-compatible providers that expose `GET /models`, enable dynamic model discovery so OpenCode loads model IDs automatically at startup.
...
If your endpoint is not `<baseURL>/models`, set `options.modelsURL`.
